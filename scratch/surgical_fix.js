...
if (content.indexOf('export async function publishChapterAction') === -1) {
    const insertIndex = content.indexOf('export async function saveMangaAction');
    if (insertIndex !== -1) {
        let startIndex = content.lastIndexOf('/**', insertIndex);
        if (startIndex !== -1) {
            content = content.slice(0, startIndex) + publishChapterAction + '\n\n' + content.slice(startIndex);
        }
    }
}

// Add /latest revalidation to saveMangaAction if missing
if (content.includes('saveMangaAction') && !content.includes("revalidatePath('/latest')")) {
    // Look for the revalidatePath part in saveMangaAction
    const searchStr = "revalidatePath('/');\n      if (mangaId) revalidatePath(`/manga/${mangaId}`);";
    if (content.includes(searchStr)) {
        content = content.replace(searchStr, "revalidatePath('/');\n      revalidatePath('/latest');\n      if (mangaId) revalidatePath(`/manga/${mangaId}`);");
    }
}

fs.writeFileSync(filePath, content);
console.log('Surgically updated actions.js');
