          ...
          <AnimatePresence mode="wait">
              ...
              <motion.div key="profile" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-10">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="bg-[#141814] p-8 rounded-[40px] border border-white/5 flex flex-col items-center gap-4">
                    <span className="text-[10px] font-black text-[#4caf50] uppercase tracking-widest">Chuỗi điểm danh</span>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="bg-[#141814] p-6 rounded-3xl border border-white/5 text-center">
                      <p className="text-[10px] text-gray-500 uppercase font-black">Truyện</p>
                    ...
                    <div className="bg-[#141814] p-6 rounded-3xl border border-white/5 text-center col-span-2">
                       <p className="text-[10px] text-gray-500 uppercase font-black">Cấp độ</p>
                       <p className="text-3xl font-black text-[#4caf50]">{calculateLevel(user?.xp)}</p>
                  ...
                  <div className="space-y-3 max-h-[300px] overflow-y-auto pr-2 custom-scrollbar">
                    {xpLogs.map(l => (
                      <div key={l.id} className="flex justify-between items-center p-4 bg-black/40 rounded-2xl border border-white/5">
                        <div className="text-[10px] font-black uppercase text-gray-400">{l.type}</div>
            ...
            {activeTab === 'settings' && (
              <motion.div key="settings" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-8">
                <form onSubmit={handleUpdate} className="bg-[#141814] p-8 rounded-[40px] border border-white/5 space-y-6">
                  <h3 className="text-xs font-black uppercase tracking-widest">Cập nhật hồ sơ</h3>
                  <div className="space-y-4">
                    <div className="space-y-1">
                      <label className="text-[10px] font-black text-gray-500 uppercase ml-2">Tên hiển thị</label>
                      <input type="text" value={displayName} onChange={e => setDisplayName(e.target.value)} className="w-full bg-black/40 border border-white/5 rounded-xl px-4 py-3 outline-none focus:border-[#4caf50]" />
                    </div>
                    <div className="space-y-1">
                      <label className="text-[10px] font-black text-gray-500 uppercase ml-2">Giới thiệu</label>
                      <textarea value={bio} onChange={e => setBio(e.target.value)} className="w-full bg-black/40 border border-white/5 rounded-xl px-4 py-3 outline-none focus:border-[#4caf50] resize-none" rows="3" />
                    </div>
                    <button type="submit" disabled={updating} className="w-full py-4 bg-[#4caf50] text-[#0a0c0a] font-black rounded-xl text-[10px] uppercase shadow-lg disabled:opacity-20">Lưu thay đổi</button>
                    {message && <p className="text-center text-[10px] font-black text-[#4caf50]">{message}</p>}
                  </div>
                </form>

                <form onSubmit={handlePasswordUpdate} className="bg-[#141814] p-8 rounded-[40px] border border-white/5 space-y-6">
                  <h3 className="text-xs font-black uppercase tracking-widest">Đổi mật mã</h3>
                  <div className="space-y-4">
                    <input type="password" placeholder="Mật khẩu cũ" value={oldPassword} onChange={e => setOldPassword(e.target.value)} className="w-full bg-black/40 border border-white/5 rounded-xl px-4 py-3 outline-none focus:border-[#4caf50]" />
                    ...
                    <input type="password" placeholder="Xác nhận" value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)} className="w-full bg-black/40 border border-white/5 rounded-xl px-4 py-3 outline-none focus:border-[#4caf50]" />
                    <button type="submit" disabled={pwdUpdating} className="w-full py-4 bg-white/5 text-white font-black rounded-xl text-[10px] uppercase border border-white/10 hover:bg-white/10 disabled:opacity-20">Thay đổi mật mã</button>
            ...
            {activeTab === 'notifications' && (
              <motion.div key="notifications" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-4">
                  <div className="bg-[#141814] p-8 rounded-[40px] border border-white/5 space-y-6">
                     <h3 className="text-xs font-black uppercase tracking-widest">Thông báo của bạn</h3>
                     <div className="space-y-3 max-h-[500px] overflow-y-auto pr-2 custom-scrollbar">
                        {notifications.map(n => (
                          <div key={n.id} onClick={() => !n.is_read && handleMarkAsRead(n.id)} className={`p-5 rounded-2xl border transition-all cursor-pointer ${!n.is_read ? 'bg-[#4caf50]/5 border-[#4caf50]/20' : 'bg-black/20 border-white/5'}`}>
                            <p className="text-[11px] font-black uppercase text-white mb-1">{n.title}</p>
                        ...
                        {notifications.length === 0 && <p className="text-center text-gray-600 py-10 italic">Trống ✨</p>}
                     </div>
                  </div>
              </motion.div>
            )}
            ...
            {activeTab === 'admin' && user?.role === 'admin' && (
              <motion.div key="admin" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-6">
                 <div className="bg-[#141814] p-8 rounded-[40px] border border-red-500/10 space-y-6">
                    <h3 className="text-xs font-black uppercase tracking-widest text-red-500">Quản lý nhân nhân sự</h3>
                    <form onSubmit={handleSearchUsers} className="flex gap-2">
                       <input type="text" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder="Username..." className="flex-1 bg-black/40 border border-white/10 rounded-xl px-4 py-2 text-sm outline-none" />
                       <button className="bg-red-500 text-white px-6 rounded-xl font-black text-[10px]">TÌM</button>
                    </form>
                    <div className="space-y-2">
                       {foundUsers.map(u => (
                         <div key={u.id} className="p-4 bg-black/40 rounded-2xl border border-white/5 flex items-center justify-between">
                            ...
                            <div className="flex gap-1">
                               <button onClick={() => handleUpdateRole(u.id, 'staff')} className="text-[7px] px-2 py-1 bg-blue-500/10 text-blue-500 rounded border border-blue-500/20">STAFF</button>
                               <button onClick={() => handleUpdateRole(u.id, 'admin')} className="text-[7px] px-2 py-1 bg-red-500/10 text-red-500 rounded border border-red-500/20">ADMIN</button>
                               <button onClick={() => handleUpdateRole(u.id, 'user')} className="text-[7px] px-2 py-1 bg-gray-500/10 text-gray-500 rounded border border-gray-500/20">USER</button>
                            </div>
                         </div>
                       ))}
                    </div>
